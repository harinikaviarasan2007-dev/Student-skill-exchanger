...
export const getExchanges = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.userId!;
    const exchanges = db.exchanges.findByUserId(userId);

    const enriched = exchanges.map((ex) => {
      const isSender = ex.senderId === userId;
      const partnerId = isSender ? ex.receiverId : ex.senderId;
      const partner = db.getUserWithDetails(partnerId);

      // Check if current user has already reviewed this exchange
      const myReview = db.reviews.findByExchangeAndReviewer(ex.id, userId);
...
export const completeExchange = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.userId!;

    const exchange = db.exchanges.findById(id);
    if (!exchange) {
    ...
    const completed = db.exchanges.complete(id);
