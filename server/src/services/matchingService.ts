...
export interface MatchResult {
  candidate: ReturnType<typeof db.getUserWithDetails>;
  score: number; // 0 - 100%
  reasons: string[];
...
export function calculateMatchesForUser(currentUserId: string): MatchResult[] {
  const currentUser = db.getUserWithDetails(currentUserId);
  if (!currentUser) return [];

  const candidates = db.getAllUsersWithDetails(currentUserId);
  ...
  for (const candidate of candidates) {
    ...
    const theyCanTeach = candidate.offeredSkills.filter((s) =>
      currentRequiredNames.includes(s.name.toLowerCase())
    );

    // Skills current user offers that candidate wants
    const youCanTeach = currentUser.offeredSkills.filter((s) =>
      candidateRequiredNames.includes(s.name.toLowerCase())
    );
    ...
    const sameAvailability =
      currentUser.availability.toLowerCase() === candidate.availability.toLowerCase() ||
    ...
    const sameMode =
      currentUser.learningMode.toLowerCase() === candidate.learningMode.toLowerCase() ||
    ...
    if (theyCanTeach.length > 0) {
      reasons.push(
        `✓ They can teach ${theyCanTeach.map((s) => s.name).join(', ')}`
      );
    }
    if (youCanTeach.length > 0) {
      reasons.push(
        `✓ You can teach ${youCanTeach.map((s) => s.name).join(', ')}`
      );
    }
    if (sameAvailability) {
      reasons.push(
