...
export function seedDatabase() {
  ...
  skillsList.forEach((s) => db.skills.create(s));
  ...
  for (const s of students) {
    ...
    for (const skillName of s.offered) {
      const skill = db.skills.findOrCreate(skillName);
      db.userOfferedSkills.add(user.id, skill.id);
    }

    // Add required skills
    for (const skillName of s.required) {
      const skill = db.skills.findOrCreate(skillName);
      db.userRequiredSkills.add(user.id, skill.id);
    }
  }
  ...
  if (priyaId && vikramId) {
    const req1 = db.exchangeRequests.create({
      senderId: vikramId,
      receiverId: priyaId,
    ...
    db.exchangeRequests.updateStatus(req1.id, 'COMPLETED');

    const ex1 = db.exchanges.create({
      requestId: req1.id,
      senderId: vikramId,
      receiverId: priyaId,
      offeredSkillName: 'Mathematics',
  ...
  if (snehaId && rahulId) {
    const req2 = db.exchangeRequests.create({
      senderId: rahulId,
      receiverId: snehaId,
    ...
    db.exchangeRequests.updateStatus(req2.id, 'COMPLETED');

    const ex2 = db.exchanges.create({
      requestId: req2.id,
      senderId: rahulId,
      receiverId: snehaId,
      offeredSkillName: 'Java',
